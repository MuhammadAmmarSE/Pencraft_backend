...
export class BillingService {
  ...
  // ─── Step 1: Create a recurring charge on Shopify ─────────────────────────
  // Returns confirmationUrl — redirect merchant here to approve billing
  async createSubscription(
    ...
    // Shopify GraphQL — create a recurring application charge
    const mutation = `
    ...
    const variables = {
      ...
      // Use test mode in non-production — no real charges
    ...
    // Store pending charge ID on the shop record
    await shop.update({ shopifyChargeId: chargeId });

    this.logger.log(`Subscription created for ${shop.shopDomain} → plan: ${planType}, chargeId: ${chargeId}`);

    return { confirmationUrl, chargeId };
  }

  // ─── Step 2: Callback after merchant approves billing ─────────────────────
  // Shopify redirects here with ?charge_id= after merchant approves
  async handleCallback(
    ...
    // Verify the charge is active via Shopify GraphQL
    ...
    // Activate the plan — reset monthly counter and store charge ID
    await shop.update({
      plan: planType,
  ...
  // ─── Step 3: Verify subscription status with Shopify ──────────────────────
  ...
  // ─── Cancel subscription on Shopify ──────────────────────────────────────
  ...
  // ─── Handle Shopify billing webhook (subscription updated/cancelled) ───────
  async handleSubscriptionWebhook(
    ...
    // If Shopify cancels/expires the subscription — downgrade gracefully
  ...
  // ─── Monthly usage reset (called by a cron or scheduled job) ─────────────
