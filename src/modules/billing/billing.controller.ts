...
export class BillingController {
  ...
  async subscribe(@CurrentShop() shop: Shop, @Body('plan') plan: string) {
    ...
    const result = await this.billingService.createSubscription(shop, plan as PlanType);
  ...
  async billingCallback(
    ...
    try {
      await this.billingService.handleCallback(shopDomain, plan as PlanType, chargeId);

      this.logger.log(`Billing callback success: ${shopDomain} → ${plan}`);

      return res.redirect(`${frontendUrl}/upgrade?success=true&plan=${plan}`);
    } catch (error: any) {
  ...
  @HttpCode(HttpStatus.OK)
  async handleSubscriptionWebhook(@Req() req: Request, @Body() body: any) {
