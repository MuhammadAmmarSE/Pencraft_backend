...
export class CopywriterService {
  ...
  async rewriteProduct(shop: Shop, dto: RewriteProductDto): Promise<RewriteJob> {
    ...
    try {
      ...
      await job.update({ status: JobStatus.FAILED, errorMessage: error.message });
      throw error;
    }
  }
  ...
  async applyRewriteToShopify(shop: Shop, jobId: number): Promise<RewriteJob> {
    ...
    await job.update({ appliedToShopify: true, appliedAt: new Date() });
  ...
  async getJob(shop: Shop, jobId: number): Promise<RewriteJob> {
    const job = await this.rewriteJobModel.findOne({
      where: { id: jobId, shopId: shop.id },
    });
